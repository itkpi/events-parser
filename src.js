...
target="_blank">SEARCH IMAGE</a><br/>${title}<br/>${agenda}`',
  ...
  ain: {
    ...
    NUEeventId: 'link', 
    NUEsrcLink: 'link',
    eventData: `cheerio.load(request('GET', data).getBody().toString())`,
    title: 'ainTitle(data)',
    agenda: `data('.txt').html()`,
    addInfo: '`<a href="${link}">ORIGINAL POST</a> | <br/>${title}<br/>${agenda}`',
    place: `data('.address-marker').text() + ' ' + data('div.ven').next().text()`,
    registration: `data('.event-head').find('a').attr('href')`,
    image: 'ainImage(data)',
    dateStart: "dateFromMilliseconds(src, \'ainDate(srcName, data)\')",
    dateEnd: "dateFromMilliseconds(src, \'ainDate(srcName, data)\')",
    timeStart: "timeFromMilliseconds(src, \'ainDate(srcName, data)\')",
    timeEnd: "timeFromMilliseconds(src, \'ainDate(srcName, data)\')"
  }
}
